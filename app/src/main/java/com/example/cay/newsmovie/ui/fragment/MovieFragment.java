 ...
 * 电影
 */
public class MovieFragment extends BaseFragment<FragmentMovieBinding> implements BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener  {
    ...
    private void loadTopbarData() {
        OkHttpUtils.get().url("http://60.205.183.88:8080/VMovie/ServerTopbarData").addParams("type", "movie").build().execute(new StringCallback() {
            @Override
    ...
    private void httpGetData(final String position1, String num, final boolean first) {
        OkHttpUtils.get().url("http://60.205.183.88:8080/VMovie/MuchFindDataServer").addParams("type1", "type").addParams("value1", "2").addParams("position", position1).addParams("num", num).build().execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
                if (first) {
                    showError();
    ...
    public void onRefresh() {
        ...
        if (isFirst) {
            ...
            OkHttpUtils.get().url("http://60.205.183.88:8080/VMovie/MuchFindDataServer").addParams("type1", "type").addParams("value1", "2").addParams("position", "0").addParams("num", "15").build().execute(new StringCallback() {
                @Override
                public void onError(Call call, Exception e, int id) {
                    showError();
                }
